...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet
{
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet =
  {
    ...
    val accElem = if (p(elem)) accLeft.incl(elem) else accLeft
    val accRight = right.filterAcc(p, accElem)
    accRight
  }
  ...
  {
    right.mostRetweetedAcc(left.mostRetweetedAcc(if (elem.retweets > t.retweets) elem else t))
  }

  def descendingByRetweet: TweetList =
  {
    val t = mostRetweeted
    val l = remove(t).descendingByRetweet
    new Cons(t, l)
  }
