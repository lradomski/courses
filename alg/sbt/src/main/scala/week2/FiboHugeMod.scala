...
//    println( fiboHugeMod(99999999999999999L, 100*1000) )
  ...
  def fiboHugeMod2(n: Long, mod: Int) : Int =
  {
    require(n > 0, "n > 0")
...
//          if (i % 500 == 0) println(i)
      ...
      if (i < n)
      {
        ...
        while (i < n && !periodEnd(i, period) && periodRepeats(i, period))
        {
//          if (i % 500 == 0) println(i)
          i+=1
    ...
    } while (!periodEnd(i, period) && i < n)

    if (periodEnd(i, period)) fibo( (n % period).toInt ) else fiboMod(n, mod)
  }
